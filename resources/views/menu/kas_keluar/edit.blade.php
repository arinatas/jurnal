			...
			<div id="kt_content_container" class="container-xxl">
				...
				<div class="card">
					...
					<div class="card-body pb-5">
						...
						<!--begin::Table-->
                        <div class="mt-15">
                            <form action="{{ route('update.kasKeluar', $kasKeluar->id ) }}" method="POST">
                                @csrf
								<div class="mb-10">
                                    <label for="exampleFormControlInput1" class="required form-label">Periode Jurnal</label>
                                    <input type="date" value="{{$kasKeluar->periode_jurnal}}" class="form-control form-control-solid" required name="periode_jurnal"/>
                                </div>
								<div class="mb-10">
									...
									<select class="form-select form-select-solid" required data-control="select2" data-hide-search="true" data-placeholder="Pilih Type Jurnal" name="type_jurnal">
										...
										<option value="ju" {{$kasKeluar->type_jurnal == "ju" ? 'selected' : ''}}>Jurnal Umum</option>
										<option value="jp" {{$kasKeluar->type_jurnal == "jp" ? 'selected' : ''}}>Jurnal Penyesuaian</option>
									</select>
                                </div>
								<div class="mb-10">
									...
									<select class="form-select form-control-solid" name="divisi" required>
										...
										@foreach($divisions as $division)
											<option value="{{$division->id}}" {{$kasKeluar->divisi == $division->id ? 'selected' : ''}}>{{$division->nama_divisi}}</option>
										@endforeach
									</select>
								</div>
								<div class="mb-10">
									...
									<select class="form-select form-control-solid" name="kode_akun" required>
										...
										@foreach($jurnalAkuns as $jurnalAkun)
											<option value="{{$jurnalAkun->no_akun}}" {{$kasKeluar->kode_akun == $jurnalAkun->no_akun ? 'selected' : ''}}>{{$jurnalAkun->no_akun}} - {{$jurnalAkun->nama_akun}}</option>
										@endforeach
									</select>
								</div>
								<div class="mb-10">
                                    <label for="exampleFormControlInput1" class="required form-label">Uraian</label>
                                    <input type="text" value="{{$kasKeluar->uraian}}" class="form-control form-control-solid" required name="uraian"/>
                                </div>
								<div class="mb-10">
                                    <label for="exampleFormControlInput1" class="required form-label">No Bukti</label>
                                    <input type="text" value="{{$kasKeluar->no_bukti}}" class="form-control form-control-solid" required name="no_bukti"/>
                                </div>
								<div class="mb-10">
                                    <label for="exampleFormControlInput1" class="required form-label">Debit</label>
                                    <input type="text" value="{{$kasKeluar->debit}}" class="form-control form-control-solid" required name="debit"/>
                                </div>
								<div class="mb-10">
                                    <label for="exampleFormControlInput1" class="required form-label">Kredit</label>
                                    <input type="text" value="{{$kasKeluar->kredit}}" class="form-control form-control-solid" required name="kredit"/>
                                </div>
								<div class="mb-10">
                                ...
                                <div class="d-flex justify-content-end">
                                    <!--begin::Actions-->
                                    <a href="{{ route('kasKeluar') }}" class="btn btn-secondary">
